...
public class View extends JPanel {
	     ...
	     private Line2D verticalSegmentLine = new Line2D.Double(135,32,135,681);
		...
		public void paint(Graphics g) {
			...
			for (int i = 0; i < dim; i++){	//Cheating a bit by taking out the last line so it doesn't look ugly
				g2D.translate(segmentLineDistance, 0);	// <------ bekomme es einfach nicht hin das die Abschnitte gleich verteilt sind >.<
			...
			for (String l : model.getLabels()) {
				g2D.drawString(l,220+i,25);	//Draws labels starting at position and height
